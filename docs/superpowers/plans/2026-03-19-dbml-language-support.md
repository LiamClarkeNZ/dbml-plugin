...
Most token type constants are generated by Grammar-Kit into `DbmlTypes` (in `src/main/gen`). `DbmlTokenTypes` only holds tokens NOT declared in the BNF `tokens` block:
...
object DbmlTokenTypes {
    // Tokens not in the BNF grammar — the lexer returns these but the parser skips them
    @JvmField val NEWLINE = DbmlTokenType("NEWLINE")

    // Standard IntelliJ types (used by JFlex directly)
...
Note: This file imports from `DbmlTypes` (Grammar-Kit generated) for most tokens, and from `DbmlTokenTypes` only for `NEWLINE` and `WHITE_SPACE`.
...
- **Token identity:** The flex file imports from `nz.co.steelsky.dbmlplugin.psi.DbmlTypes` (the Grammar-Kit generated type holder), NOT from `DbmlTokenTypes`. This is critical — the parser checks against `DbmlTypes.TABLE` etc., so the lexer must return the same `IElementType` instances. `DbmlTokenTypes` only holds tokens NOT declared in the BNF: `WHITE_SPACE`, `BAD_CHARACTER`, and `NEWLINE`.
