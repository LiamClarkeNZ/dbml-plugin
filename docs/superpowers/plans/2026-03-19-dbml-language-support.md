...
Reference: `~/dev/dbml-java/src/main/java/com/wn/dbml/compiler/token/TokenType.java`
...
object DbmlTokenTypes {
    // Keywords
    @JvmField val PROJECT = DbmlTokenType("PROJECT")
    @JvmField val TABLE = DbmlTokenType("TABLE")
    @JvmField val AS = DbmlTokenType("AS")
    @JvmField val REF = DbmlTokenType("REF")
    @JvmField val ENUM = DbmlTokenType("ENUM")
    @JvmField val TABLEGROUP = DbmlTokenType("TABLEGROUP")
    @JvmField val TABLEPARTIAL = DbmlTokenType("TABLEPARTIAL")
    @JvmField val HEADERCOLOR = DbmlTokenType("HEADERCOLOR")
    @JvmField val COLOR = DbmlTokenType("COLOR")
    @JvmField val NOTE = DbmlTokenType("NOTE")
    @JvmField val PRIMARY = DbmlTokenType("PRIMARY")
    @JvmField val KEY = DbmlTokenType("KEY")
    @JvmField val PK = DbmlTokenType("PK")
    @JvmField val NULL = DbmlTokenType("NULL")
    @JvmField val NOT = DbmlTokenType("NOT")
    @JvmField val UNIQUE = DbmlTokenType("UNIQUE")
    @JvmField val DEFAULT = DbmlTokenType("DEFAULT")
    @JvmField val INCREMENT = DbmlTokenType("INCREMENT")
    @JvmField val INDEXES = DbmlTokenType("INDEXES")
    @JvmField val BTREE = DbmlTokenType("BTREE")
    @JvmField val HASH = DbmlTokenType("HASH")
    @JvmField val TYPE = DbmlTokenType("TYPE")
    @JvmField val NAME = DbmlTokenType("NAME")
    @JvmField val DELETE = DbmlTokenType("DELETE")
    @JvmField val UPDATE = DbmlTokenType("UPDATE")
    @JvmField val CASCADE = DbmlTokenType("CASCADE")
    @JvmField val RESTRICT = DbmlTokenType("RESTRICT")
    @JvmField val SET = DbmlTokenType("SET")
    @JvmField val NO = DbmlTokenType("NO")
    @JvmField val ACTION = DbmlTokenType("ACTION")

    // Operators and delimiters
    @JvmField val MINUS = DbmlTokenType("MINUS")
    @JvmField val LT = DbmlTokenType("LT")
    @JvmField val GT = DbmlTokenType("GT")
    @JvmField val NE = DbmlTokenType("NE")
    @JvmField val LPAREN = DbmlTokenType("LPAREN")
    @JvmField val RPAREN = DbmlTokenType("RPAREN")
    @JvmField val LBRACK = DbmlTokenType("LBRACK")
    @JvmField val RBRACK = DbmlTokenType("RBRACK")
    @JvmField val LBRACE = DbmlTokenType("LBRACE")
    @JvmField val RBRACE = DbmlTokenType("RBRACE")
    @JvmField val COLON = DbmlTokenType("COLON")
    @JvmField val COMMA = DbmlTokenType("COMMA")
    @JvmField val DOT = DbmlTokenType("DOT")
    @JvmField val TILDE = DbmlTokenType("TILDE")

    // Literals
    @JvmField val LITERAL = DbmlTokenType("LITERAL")
    @JvmField val NUMBER = DbmlTokenType("NUMBER")

    // Strings
    @JvmField val SINGLE_QUOTED_STRING = DbmlTokenType("SINGLE_QUOTED_STRING")
    @JvmField val DOUBLE_QUOTED_STRING = DbmlTokenType("DOUBLE_QUOTED_STRING")
    @JvmField val TRIPLE_QUOTED_STRING = DbmlTokenType("TRIPLE_QUOTED_STRING")
    @JvmField val EXPRESSION = DbmlTokenType("EXPRESSION")

    // Other
    @JvmField val COLOR_CODE = DbmlTokenType("COLOR_CODE")
    @JvmField val LINE_COMMENT = DbmlTokenType("LINE_COMMENT")
    @JvmField val BLOCK_COMMENT = DbmlTokenType("BLOCK_COMMENT")
    @JvmField val NEWLINE = DbmlTokenType("NEWLINE")

    // Standard IntelliJ types
...
object DbmlTokenSets {
    @JvmField
    val COMMENTS = TokenSet.create(DbmlTokenTypes.LINE_COMMENT, DbmlTokenTypes.BLOCK_COMMENT)

    @JvmField
    val STRINGS = TokenSet.create(
        DbmlTokenTypes.SINGLE_QUOTED_STRING,
        DbmlTokenTypes.DOUBLE_QUOTED_STRING,
        DbmlTokenTypes.TRIPLE_QUOTED_STRING,
    )

    @JvmField
    val KEYWORDS = TokenSet.create(
        DbmlTokenTypes.PROJECT, DbmlTokenTypes.TABLE, DbmlTokenTypes.AS,
        DbmlTokenTypes.REF, DbmlTokenTypes.ENUM, DbmlTokenTypes.TABLEGROUP,
        DbmlTokenTypes.TABLEPARTIAL, DbmlTokenTypes.HEADERCOLOR, DbmlTokenTypes.COLOR,
        DbmlTokenTypes.NOTE, DbmlTokenTypes.PRIMARY, DbmlTokenTypes.KEY,
        DbmlTokenTypes.PK, DbmlTokenTypes.NULL, DbmlTokenTypes.NOT,
        DbmlTokenTypes.UNIQUE, DbmlTokenTypes.DEFAULT, DbmlTokenTypes.INCREMENT,
        DbmlTokenTypes.INDEXES, DbmlTokenTypes.BTREE, DbmlTokenTypes.HASH,
        DbmlTokenTypes.TYPE, DbmlTokenTypes.NAME, DbmlTokenTypes.DELETE,
        DbmlTokenTypes.UPDATE, DbmlTokenTypes.CASCADE, DbmlTokenTypes.RESTRICT,
        DbmlTokenTypes.SET, DbmlTokenTypes.NO, DbmlTokenTypes.ACTION,
    )
...
- [ ] **Step 4: Verify compilation of token types**
...
```bash
git add -A
git commit -m "feat: add DBML lexer token types, token sets, and adapter"
```
...
import static nz.co.steelsky.dbmlplugin.lexer.DbmlTokenTypes.*;
...
<YYINITIAL> {
    ...
    "'''"                       { yybegin(IN_TRIPLE_STRING); return TRIPLE_QUOTED_STRING; }
    "'"                         { yybegin(IN_SINGLE_STRING); return SINGLE_QUOTED_STRING; }
    "\""                        { yybegin(IN_DOUBLE_STRING); return DOUBLE_QUOTED_STRING; }
    "`"                         { yybegin(IN_EXPRESSION); return EXPRESSION; }
...
// === String states ===
// Each state uses yymore() to accumulate content, returning a single token on close.
// The opening delimiter was already returned from YYINITIAL; yymore() appends to it,
// and the closing delimiter triggers yybegin(YYINITIAL) + return.
// For IntelliJ JFlex lexers, the standard approach is to NOT return until the closing
// delimiter, so each string literal is one token. If EOF is hit, return BAD_CHARACTER.
...
- String and comment states do NOT return a token until the closing delimiter. JFlex rules without a return statement match text but continue lexing. The opening delimiter token was returned from `YYINITIAL`; the closing delimiter returns the token type for the whole literal. The `zzStartRead` position (set when the opening delimiter was matched) will cover the entire string.
- **Correction:** Actually, in IntelliJ JFlex lexers, each `advance()` call must return a token. The above approach of not returning will not work with `FlexAdapter`. The correct approach is: the opening delimiter transitions to the state but does NOT return a token; instead the state accumulates until the close and returns. To achieve this, change the YYINITIAL string rules to NOT return — just `yybegin(STATE);` — and have the state return the token type on close. Alternatively, the simpler approach is to match entire string literals in YYINITIAL with regex patterns. For v1, use this simpler approach if the state-based one proves tricky. See the IntelliJ Simple Language tutorial for a reference.
...
ref_column_names ::= table_name DOT (identifier_ | LPAREN identifier_ (COMMA identifier_)* RPAREN)
...
import nz.co.steelsky.dbmlplugin.lexer.DbmlLexerAdapter
import nz.co.steelsky.dbmlplugin.lexer.DbmlTokenSets
import nz.co.steelsky.dbmlplugin.lexer.DbmlTokenTypes
...
class DbmlSyntaxHighlighter : SyntaxHighlighterBase() {
    ...
    override fun getTokenHighlights(tokenType: IElementType): Array<TextAttributesKey> {
        return when {
            ...
            tokenType == DbmlTokenTypes.NUMBER -> NUMBER_KEYS
            DbmlTokenSets.STRINGS.contains(tokenType) -> STRING_KEYS
            tokenType == DbmlTokenTypes.EXPRESSION -> EXPRESSION_KEYS
            tokenType == DbmlTokenTypes.LINE_COMMENT -> LINE_COMMENT_KEYS
            tokenType == DbmlTokenTypes.BLOCK_COMMENT -> BLOCK_COMMENT_KEYS
            tokenType == DbmlTokenTypes.COLOR_CODE -> COLOR_CODE_KEYS
            tokenType == DbmlTokenTypes.LBRACE || tokenType == DbmlTokenTypes.RBRACE -> BRACES_KEYS
            tokenType == DbmlTokenTypes.LBRACK || tokenType == DbmlTokenTypes.RBRACK -> BRACKETS_KEYS
            tokenType == DbmlTokenTypes.LPAREN || tokenType == DbmlTokenTypes.RPAREN -> PARENTHESES_KEYS
            tokenType == DbmlTokenTypes.MINUS || tokenType == DbmlTokenTypes.LT
                || tokenType == DbmlTokenTypes.GT || tokenType == DbmlTokenTypes.NE
                || tokenType == DbmlTokenTypes.COLON || tokenType == DbmlTokenTypes.COMMA
                || tokenType == DbmlTokenTypes.DOT || tokenType == DbmlTokenTypes.TILDE -> OPERATOR_KEYS
...
class DbmlHighlightingTest : BasePlatformTestCase() {
    ...
    fun testKeywordsHighlighted() {
        val keywordTokens = listOf(
            DbmlTokenTypes.TABLE, DbmlTokenTypes.PROJECT, DbmlTokenTypes.REF,
            DbmlTokenTypes.ENUM, DbmlTokenTypes.NOTE, DbmlTokenTypes.INDEXES,
        )
    ...
    fun testCommentHighlighted() {
        assertHighlightContains(DbmlTokenTypes.LINE_COMMENT, DbmlSyntaxHighlighter.LINE_COMMENT)
        assertHighlightContains(DbmlTokenTypes.BLOCK_COMMENT, DbmlSyntaxHighlighter.BLOCK_COMMENT)
    }

    fun testNumberHighlighted() {
        assertHighlightContains(DbmlTokenTypes.NUMBER, DbmlSyntaxHighlighter.NUMBER)
    }

    fun testOperatorHighlighted() {
        val operatorTokens = listOf(
            DbmlTokenTypes.LT, DbmlTokenTypes.GT, DbmlTokenTypes.MINUS,
            DbmlTokenTypes.NE, DbmlTokenTypes.COLON, DbmlTokenTypes.DOT,
        )
    ...
    fun testBracesHighlighted() {
        assertHighlightContains(DbmlTokenTypes.LBRACE, DbmlSyntaxHighlighter.BRACES)
        assertHighlightContains(DbmlTokenTypes.RBRACE, DbmlSyntaxHighlighter.BRACES)
    }
...
class DbmlBraceMatcher : PairedBraceMatcher {
    companion object {
        private val PAIRS = arrayOf(
            BracePair(DbmlTokenTypes.LBRACE, DbmlTokenTypes.RBRACE, true),
            BracePair(DbmlTokenTypes.LBRACK, DbmlTokenTypes.RBRACK, false),
            BracePair(DbmlTokenTypes.LPAREN, DbmlTokenTypes.RPAREN, false),
        )
    }
